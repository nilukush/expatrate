...
export function sitemapXml(urls: string[]): string {
  ...
  const groups = new Map<string, { en?: string; ar?: string; hi?: string; id?: string; es?: string }>();
  for (const url of urls) {
    const match = url.match(/^https?:\/\/[^/]+(\/(ar|hi|id|es))(\/.*)?$/);
    const locale = match ? match[2] : 'en';
    const path = match ? match[3] ?? '/' : url.replace(/^https?:\/\/[^/]+/, '') || '/';
    const group = groups.get(path) ?? {};
    group[locale as 'en' | 'ar' | 'hi' | 'id' | 'es'] = url;
  ...
  const entries = urls
    .map((url) => {
      ...
      const alternates = (['en', 'ar', 'hi', 'id', 'es'] as const)
        .filter((l) => group[l])
...
export function allSiteUrls(): string[] {
  ...
  for (const locale of ['', '/ar', '/hi', '/id', '/es']) {
