...
Launched 31 August 2026 at https://expatrate.pages.dev. Live coverage: 68 countries with verified market benchmarks (1,715 curated rows; all 27 EU member states plus Iceland, Norway, and Switzerland, with Liechtenstein the one EEA market still without benchmarks) and 2026 tax treatment (statutory schedules for 27 countries, from full bracket regimes to the Gulf zero-tax states), a purchasing-power floor for all 207 supported countries (Taiwan via the IMF, the rest via the World Bank), English, Arabic, and Hindi locales with full RTL. The gate (typecheck, 167 unit tests, 60 Playwright end to end tests, build) runs in CI on every push.
