...
test('Big-market SES spine (2026-09-07): Germany, France, Ireland, Switzerland move off recruiter and job-board sources', () => {
  ...
  expect(load('benchmarks.json').entries.filter((e: { status?: string }) => e.status === undefined).length).toBe(3_089);
});
...
test('Ukraine joins on the Derzhstat Data Bank SDMX API (2026-09-09): Q2 2026 average monthly wage by activity section', () => {
  ...
  const ukr = by('UKR');
  expect(ukr.length).toBe(11);
  for (const row of ukr) {
    expect(row.currency).toBe('UAH');
    expect(row.basis).toBe('monthly-gross');
    expect(row.quality).toBe('Medium');
    expect(row.p25).toBe(0);
    expect(row.p75).toBe(0);
    expect((row.sources ?? []).some((s: string) => s.includes('stat.gov.ua'))).toBe(true);
    expect(row.note).toContain('published MEAN');
    expect(row.level).toBe('senior');
  }
  const anchor = (family: string, value: number) => {
    const row = ukr.find((r: { family: string }) => r.family === family);
    expect(row, `UKR ${family}`).toBeDefined();
    expect(row!.p50).toBe(value);
  };
  anchor('software-engineering', 77_598);
  anchor('data-and-ai', 77_598);
  anchor('cybersecurity', 77_598);
  anchor('finance-and-accounting', 64_944);
  anchor('sales-and-business-development', 36_686);
  anchor('marketing-and-growth', 39_836);
  anchor('hr-and-people', 24_776);
  anchor('operations-and-supply-chain', 29_647);
  anchor('engineering-civil-mechanical-electrical', 25_060);
  anchor('healthcare', 21_564);
...
test('Tajikistan joins on the stat.tj analytical tables workbook (2026-09-09): 2024 average monthly wage by sector', () => {
  ...
  const tjk = by('TJK');
  expect(tjk.length).toBe(11);
  for (const row of tjk) {
    expect(row.currency).toBe('TJS');
    expect(row.basis).toBe('monthly-gross');
    expect(row.quality).toBe('Medium');
    expect(row.p25).toBe(0);
    expect(row.p75).toBe(0);
    expect((row.sources ?? []).some((s: string) => s.includes('stat.tj'))).toBe(true);
    expect(row.note).toContain('published MEAN');
    expect(row.level).toBe('senior');
  }
  const anchor = (family: string, value: number) => {
    const row = tjk.find((r: { family: string }) => r.family === family);
    expect(row, `TJK ${family}`).toBeDefined();
    expect(row!.p50).toBe(value);
  };
  anchor('software-engineering', 4_609.84);
  anchor('data-and-ai', 4_609.84);
  anchor('cybersecurity', 4_609.84);
  anchor('finance-and-accounting', 6_199.3);
  anchor('sales-and-business-development', 1_862.71);
  anchor('marketing-and-growth', 2_587.04);
  anchor('hr-and-people', 1_745.91);
  anchor('operations-and-supply-chain', 3_137.28);
  anchor('engineering-civil-mechanical-electrical', 3_072.36);
  anchor('healthcare', 1_691.82);
...
test('ILOSTAT resweep wave (2026-09-09): Jordan, Rwanda, Senegal, Uruguay 2024 plus Guatemala 2023 and Ghana 2017, medians by ISIC4 section', () => {
  ...
  const CASES: Array<{
    cc: string; cur: string; rows: number; year: string; flagged: boolean; age: boolean;
    anchors: Array<[string, string, number]>;
  }> = [
    {
      cc: 'JOR', cur: 'JOD', rows: 14, year: '2024', flagged: false, age: false,
      anchors: [
        ['software-engineering', 'senior', 450], ['data-and-ai', 'senior', 450], ['cybersecurity', 'senior', 450],
        ['finance-and-accounting', 'senior', 500], ['sales-and-business-development', 'senior', 290],
        ['marketing-and-growth', 'senior', 450], ['hr-and-people', 'senior', 280],
        ['operations-and-supply-chain', 'senior', 350], ['engineering-civil-mechanical-electrical', 'senior', 312],
        ['healthcare', 'senior', 450], ['education-and-teaching', 'senior', 430],
        ['it-executive', 'lead', 600], ['finance-and-accounting', 'lead', 600], ['general-management', 'lead', 600],
      ],
    },
    {
      cc: 'RWA', cur: 'RWF', rows: 14, year: '2024', flagged: false, age: false,
      anchors: [
        ['software-engineering', 'senior', 150_000], ['data-and-ai', 'senior', 150_000], ['cybersecurity', 'senior', 150_000],
        ['finance-and-accounting', 'senior', 200_000], ['sales-and-business-development', 'senior', 60_000],
        ['marketing-and-growth', 'senior', 152_396.33], ['hr-and-people', 'senior', 40_000],
        ['operations-and-supply-chain', 'senior', 52_000], ['engineering-civil-mechanical-electrical', 'senior', 78_000],
        ['healthcare', 'senior', 198_711.52], ['education-and-teaching', 'senior', 121_000],
        ['it-executive', 'lead', 300_000], ['finance-and-accounting', 'lead', 300_000], ['general-management', 'lead', 300_000],
      ],
    },
    {
      cc: 'SEN', cur: 'XOF', rows: 14, year: '2024', flagged: false, age: false,
      anchors: [
        ['software-engineering', 'senior', 145_270.04], ['data-and-ai', 'senior', 145_270.04], ['cybersecurity', 'senior', 145_270.04],
        ['finance-and-accounting', 'senior', 150_000], ['sales-and-business-development', 'senior', 75_000],
        ['marketing-and-growth', 'senior', 269_740.35], ['hr-and-people', 'senior', 128_887.83],
        ['operations-and-supply-chain', 'senior', 90_000], ['engineering-civil-mechanical-electrical', 'senior', 86_666.66],
        ['healthcare', 'senior', 150_000], ['education-and-teaching', 'senior', 300_000],
        ['it-executive', 'lead', 241_159.15], ['finance-and-accounting', 'lead', 241_159.15], ['general-management', 'lead', 241_159.15],
      ],
    },
    {
      cc: 'URY', cur: 'UYU', rows: 14, year: '2024', flagged: false, age: false,
      anchors: [
        ['software-engineering', 'senior', 62_732.38], ['data-and-ai', 'senior', 62_732.38], ['cybersecurity', 'senior', 62_732.38],
        ['finance-and-accounting', 'senior', 76_937.5], ['sales-and-business-development', 'senior', 33_000],
        ['marketing-and-growth', 'senior', 40_000], ['hr-and-people', 'senior', 30_000],
        ['operations-and-supply-chain', 'senior', 47_000], ['engineering-civil-mechanical-electrical', 'senior', 47_567.72],
        ['healthcare', 'senior', 42_000], ['education-and-teaching', 'senior', 45_000],
        ['it-executive', 'lead', 120_000], ['finance-and-accounting', 'lead', 120_000], ['general-management', 'lead', 120_000],
      ],
    },
    {
      cc: 'GTM', cur: 'GTQ', rows: 14, year: '2023', flagged: false, age: false,
      anchors: [
        ['software-engineering', 'senior', 4_218.04], ['data-and-ai', 'senior', 4_218.04], ['cybersecurity', 'senior', 4_218.04],
        ['finance-and-accounting', 'senior', 4_528.64], ['sales-and-business-development', 'senior', 2_503.46],
        ['marketing-and-growth', 'senior', 3_500], ['hr-and-people', 'senior', 3_725.82],
        ['operations-and-supply-chain', 'senior', 3_000], ['engineering-civil-mechanical-electrical', 'senior', 2_000],
        ['healthcare', 'senior', 3_733.33], ['education-and-teaching', 'senior', 5_000],
        ['it-executive', 'lead', 8_084.43], ['finance-and-accounting', 'lead', 8_084.43], ['general-management', 'lead', 8_084.43],
      ],
    },
    {
      cc: 'GHA', cur: 'GHS', rows: 14, year: '2017', flagged: true, age: true,
      anchors: [
        ['software-engineering', 'senior', 968.84], ['data-and-ai', 'senior', 968.84], ['cybersecurity', 'senior', 968.84],
        ['finance-and-accounting', 'senior', 751.15], ['sales-and-business-development', 'senior', 400],
        ['marketing-and-growth', 'senior', 800], ['hr-and-people', 'senior', 366.83],
        ['operations-and-supply-chain', 'senior', 335.53], ['engineering-civil-mechanical-electrical', 'senior', 800],
        ['healthcare', 'senior', 800], ['education-and-teaching', 'senior', 750],
        ['it-executive', 'lead', 1_401.47], ['finance-and-accounting', 'lead', 1_401.47], ['general-management', 'lead', 1_401.47],
      ],
    },
  ];
  for (const c of CASES) {
    const rows = by(c.cc);
    expect(rows.length, `${c.cc} rows`).toBe(c.rows);
    for (const row of rows) {
    ...
    for (const [family, level, value] of c.anchors) {
      const row = rows.find((r: { family: string; level: string }) => r.family === family && r.level === level);
      expect(row, `${c.cc} ${family} ${level}`).toBeDefined();
      expect(row!.p50).toBe(value);
    }
    expect(rows.find((r: { level: string }) => r.level === 'executive'), `${c.cc} executives`).toBeUndefined();
  }
...
test('Round 28 wave (2026-09-09): Brunei and Tonga on section medians, three leads-only markets on managers pools', () => {
  const by = (cc: string) => load('benchmarks.json').entries.filter(
    (e: { country: string; status?: string }) => e.country === cc && e.status === undefined,
  );
  const anchor = (rows: Array<{ family: string; level: string; p50: number }>, family: string, level: string, value: number, cc: string) => {
    const row = rows.find((r) => r.family === family && r.level === level);
    expect(row, `${cc} ${family} ${level}`).toBeDefined();
    expect(row!.p50).toBe(value);
  };
  // Brunei 2014: full recipe, every cell break-flagged, twelve-year conservative floor.
  const brn = by('BRN');
  expect(brn.length).toBe(14);
  for (const row of brn) {
    expect(row.currency).toBe('BND');
    expect(row.note).toContain('break-in-series');
    expect(row.note).toContain('conservative floor');
    expect((row.sources ?? []).some((s: string) => s.includes('sdmx.ilo.org'))).toBe(true);
  }
  anchor(brn, 'software-engineering', 'senior', 1_658.3, 'BRN');
  anchor(brn, 'data-and-ai', 'senior', 1_658.3, 'BRN');
  anchor(brn, 'cybersecurity', 'senior', 1_658.3, 'BRN');
  anchor(brn, 'finance-and-accounting', 'senior', 2_000, 'BRN');
  anchor(brn, 'sales-and-business-development', 'senior', 600, 'BRN');
  anchor(brn, 'marketing-and-growth', 'senior', 1_500, 'BRN');
  anchor(brn, 'hr-and-people', 'senior', 650, 'BRN');
  anchor(brn, 'operations-and-supply-chain', 'senior', 1_196.87, 'BRN');
  anchor(brn, 'engineering-civil-mechanical-electrical', 'senior', 720, 'BRN');
  anchor(brn, 'healthcare', 'senior', 1_848.78, 'BRN');
  anchor(brn, 'education-and-teaching', 'senior', 2_200, 'BRN');
  anchor(brn, 'it-executive', 'lead', 4_240, 'BRN');
  anchor(brn, 'finance-and-accounting', 'lead', 4_240, 'BRN');
  anchor(brn, 'general-management', 'lead', 4_240, 'BRN');
  // Tonga 2021: marketing cell Unreliable so no marketing senior; otherwise full.
  const ton = by('TON');
  expect(ton.length).toBe(13);
  for (const row of ton) {
    expect(row.currency).toBe('TOP');
    expect((row.sources ?? []).some((s: string) => s.includes('sdmx.ilo.org'))).toBe(true);
  }
  anchor(ton, 'software-engineering', 'senior', 765.63, 'TON');
  anchor(ton, 'data-and-ai', 'senior', 765.63, 'TON');
  anchor(ton, 'cybersecurity', 'senior', 765.63, 'TON');
  anchor(ton, 'finance-and-accounting', 'senior', 988.73, 'TON');
  anchor(ton, 'sales-and-business-development', 'senior', 800, 'TON');
  anchor(ton, 'hr-and-people', 'senior', 800, 'TON');
  anchor(ton, 'operations-and-supply-chain', 'senior', 760, 'TON');
  anchor(ton, 'engineering-civil-mechanical-electrical', 'senior', 1_200, 'TON');
  anchor(ton, 'healthcare', 'senior', 1_092.81, 'TON');
  anchor(ton, 'education-and-teaching', 'senior', 929.87, 'TON');
  anchor(ton, 'it-executive', 'lead', 1_299.13, 'TON');
  anchor(ton, 'finance-and-accounting', 'lead', 1_299.13, 'TON');
  anchor(ton, 'general-management', 'lead', 1_299.13, 'TON');
  expect(ton.find((r: { family: string }) => r.family === 'marketing-and-growth')).toBeUndefined();
  // Leads-only markets: the occupation flow publishes only one-digit groups, so the
  // managers pool anchors the three lead families and nothing else; senior absence pinned.
  const LEADS: Array<[string, string, number, string]> = [
    ['COD', 'CDF', 117_000, '2016'],
    ['DJI', 'DJF', 115_340.42, '2017'],
    ['TTO', 'TTD', 8_500, '2016'],
  ];
  for (const [cc, cur, val, year] of LEADS) {
    const rows = by(cc);
    expect(rows.length, `${cc} rows`).toBe(3);
    for (const row of rows) {
      expect(row.currency).toBe(cur);
      expect(row.level).toBe('lead');
      expect(row.note).toContain(year);
      expect(row.note).toContain('occupation major groups');
      expect((row.sources ?? []).some((s: string) => s.includes('sdmx.ilo.org'))).toBe(true);
    }
    anchor(rows, 'it-executive', 'lead', val, cc);
    anchor(rows, 'finance-and-accounting', 'lead', val, cc);
    anchor(rows, 'general-management', 'lead', val, cc);
    expect(rows.find((r: { level: string }) => r.level === 'senior'), `${cc} seniors absent`).toBeUndefined();
  }
  expect(load('benchmarks.json').entries.filter((e: { status?: string }) => e.status === undefined).length).toBe(3_089);
});
