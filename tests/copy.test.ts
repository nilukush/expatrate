...
const locales = ['en', 'ar', 'hi', 'id', 'es', 'fr', 'pt', 'ru'] as const;
const BANDS: ExperienceBand[] = ['0-2', '3-5', '6-9', '10-14', '15+'];

test('saved indicator says where the data is saved, in every locale', () => {
  const markers = { en: /browser/i, ar: 'المتصفح', hi: 'ब्राउज़र', id: /browser/i, es: /navegador/i, fr: /navigateur/i, pt: /navegador/i, ru: /браузер/i } as const;
...
test('resume upload is marked optional in its visible title and hidden label', () => {
  const markers = { en: 'optional', ar: 'اختياري', hi: 'वैकल्पिक', id: 'opsional', es: 'opcional', fr: 'facultatif', pt: 'opcional', ru: 'необязательно' } as const;
  for (const loc of locales) {
...
test('step help explains the band instead of a bare seniority sentence', () => {
  const markers = { en: 'band', ar: 'نطاق', hi: 'बैंड', id: 'rentang', es: 'banda', fr: 'bande', pt: 'faixa', ru: 'диапазон' } as const;
  for (const loc of locales) {
...
test('trust bullet scopes the country count to the floor', () => {
  const markers = { en: /floor/i, ar: 'حد', hi: 'सीमा', id: /batas/i, es: /suelo/i, fr: /plancher/i, pt: /piso/i, ru: /порог/i } as const;
  for (const loc of locales) {
...
test('footer data line names a benchmark country count, not corridors', () => {
  ...
  expect(dict('ru').home.footerData).not.toContain('коридоры');
  expect(dict('ru').home.footerData).toContain('стран назначения');
});
...
test('the job link hint names the boards and the no-server promise in every locale', () => {
  ...
  const promiseMarker = { en: /never sees it|no server/i, ar: 'دون أن يمر', hi: 'नहीं गुजरती', id: /tidak pernah melihat/i, es: /nunca lo ve/i, fr: /ne le voit jamais/i, pt: /nunca o vê/i, ru: /никогда её не видит/i } as const;
  for (const loc of locales) {
...
test('every locale carries the full localized methodology, including the 20 percent default', () => {
  const markers = {
    ...
    ru: { market: /P25/, floor: /Всемирного банка/, tax: /20 процентов/ },
  } as const;
  for (const loc of locales) {
...
test('the home trust line states the data dates in every locale', () => {
  const markers = { en: /Data as of/i, ar: /البيانات حتى/, hi: /आँकड़े अद्यतन/, id: /Data per/i, es: /Datos a/i, fr: /Données au/i, pt: /Dados de/i, ru: /Данные на/i } as const;
  for (const loc of locales) {
...
test('experience band labels carry no level jargon in any locale', () => {
  const banned = { en: 'band', ar: 'نطاق', hi: 'बैंड', id: 'rentang', es: 'banda', fr: 'bande', pt: 'faixa', ru: 'диапазон' } as const;
  for (const loc of locales) {
...
test('the role step help explains the band-to-level mapping in every locale', () => {
  const markers = { en: 'senior', ar: 'خبير', hi: 'वरिष्ठ', id: 'senior', es: 'senior', fr: 'senior', pt: 'senior', ru: 'senior' } as const;
  for (const loc of locales) {
...
test('the meta description claims your currency, not any currency', () => {
  ...
  expect(dict('ru').home.description).toContain('в вашей валюте');
});
...
test('returning visitors get a welcome-back note and an action-named resume chip in every locale', () => {
  const browserWord = { en: /browser/i, ar: 'المتصفح', hi: 'ब्राउज़र', id: /browser/i, es: /navegador/i, fr: /navigateur/i, pt: /navegador/i, ru: /браузер/i } as const;
  for (const loc of locales) {
...
test('the tax-default count in user copy derives from the data', () => {
  ...
  const expected = {
    ...
    ru: `${onDefault} из ${countries.length}`,
  } as const;
  for (const loc of locales) {
