...
export function sitemapXml(urls: string[]): string {
  ...
  const groups = new Map<string, { en?: string; ar?: string; hi?: string; id?: string; es?: string; fr?: string }>();
  for (const url of urls) {
    const match = url.match(/^https?:\/\/[^/]+(\/(ar|hi|id|es|fr))(\/.*)?$/);
    const locale = match ? match[2] : 'en';
    const path = match ? match[3] ?? '/' : url.replace(/^https?:\/\/[^/]+/, '') || '/';
    const group = groups.get(path) ?? {};
    group[locale as 'en' | 'ar' | 'hi' | 'id' | 'es' | 'fr'] = url;
  ...
  const entries = urls
    .map((url) => {
      ...
      const alternates = (['en', 'ar', 'hi', 'id', 'es', 'fr'] as const)
        .filter((l) => group[l])
...
export function allSiteUrls(): string[] {
  ...
  for (const locale of ['', '/ar', '/hi', '/id', '/es', '/fr']) {
